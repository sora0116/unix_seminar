...
== コアファイルの生成
コアファイルは実行中のプロセスのメモリイメージとプロセス状態を記録したファイルです。
`
  generate-core-file [<file>]
  gcore [<file>]
`
inferiorのコアダンプを生成します。ファイル名を指定しない場合、`core.pid`の形式で命名されます。
`
  set use-coredump-filter [on|off]
`
コアダンプ生成時に、`/proc/pid/coredump_filter`のフィルタを使用するかどうかを設定します。
`
  set dump-excluded-mappings [on|off]
`
onの場合、`VM_DONTDUMP`でマークされたメモリマッピングをダンプします。デフォルトではoffです。

== 文字セット
`
  set target-charset <charset>
`
現在のターゲットの文字セットを設定します。サポートされている文字セットは`set target-set`まで入力してTABで補完すると確認できます。
`
...
`
  set charset <charset>
`
ホスト、ターゲットの両方で文字セットを設定します。
`
  set target-wide-charset <charset>
`
ターゲットのワイド文字セットを設定します。
...
`
スタックアクセスのキャッシュを有効化/無効化します。デフォルトではonです。
`
  set code-cache [on|off]
`
コードセグメントへのアクセスのキャッシュを有効化/無効化します。デフォルトではonです。
`
  info dcache [<line>]
`
現在のinferiorのアドレス空間のデータキャッシュのパオフォーマンスに関する情報を表示します。
`
  set dcache size <size>
`
dcacheのエントリ数の最大値を設定します。
`
  set dcache line-size <line-size>
`
dcacheのエントリのバイト数を指定します。2のべきの値のみ指定できます。
`
  maint flush dcache
`
キャッシュをフラッシュします。
...
`
`val`で指定されたバイト列のメモリを検索します。検索はlenまたは`end_addr`までが対象です。`s, n`はオプションです。
- `s` : サーチクエリサイズ。以下が設定できます。
  - `b` : Byte
  - `h` : Halfwords(2Bytes)
  - `w` : words(4Bytes)
  - `g` : giant words(8Bytes)
- `n` : マッチして表示する数の最大値を設定します。

== 値のサイズ
GDBが値を表示するときにその値の内容を保持するためにメモリ領域を確保します。
`
  set max-value-size <bytes>|unlimited
`
GDBが値の内容に割り当てるメモリの最大サイズを設定します。
